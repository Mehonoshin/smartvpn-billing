...
# Setup SV for deploy user
directory "/etc/sv/runsvdir-smartvpn" do
  owner "root"
  group "root"
  mode 00755
  action :create
...
file "/etc/sv/runsvdir-smartvpn/run" do
  owner "root"
  group "root"
  mode "0755"
  action :create
  content "#!/bin/sh
  exec 2>&1
  exec chpst -udeploy runsvdir /home/deploy/services"
end

directory "/home/deploy/services" do
  owner "deploy"
  group "deploy"
  mode 00755
  action :create
...
unless File.exists?('/etc/service/smartvpn/runsvdir-smartvpn')
  execute "symlink" do
    command "ln -s /etc/sv/runsvdir-smartvpn /etc/service/smartvpn"
  end
end

directory "/home/deploy/sv" do
  owner "deploy"
  group "deploy"
  mode 00755
  action :create
...
## Services setup
['unicorn', 'sidekiq'].each do |sv|

  directory "/home/deploy/sv/#{sv}" do
    owner "deploy"
    group "deploy"
    mode 00755
    action :create
  ...
  template "/home/deploy/sv/#{sv}/run" do
    source "#{sv}-run.rb.erb"
    owner "deploy"
    group "deploy"
    mode "0755"
    action :create
  ...
  execute "symlink service" do
    command "ln -s /home/deploy/sv/#{sv} /home/deploy/services/"
  end unless File.exists?("/home/deploy/services/#{sv}")
