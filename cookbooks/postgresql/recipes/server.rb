...
execute "create-database" do
  exists = <<-EOH
  psql -U postgres -c "select * from pg_database WHERE datname='smartvpn_production'" | grep -c smartvpn_production
  EOH
  command "createdb -U postgres -O postgres -E utf8 -T template0 smartvpn_production"
  not_if exists
end
